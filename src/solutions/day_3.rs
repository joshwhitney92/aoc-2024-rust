...
impl Solution {
    pub fn solve(lines: &Vec<String>, use_delimiter: Option<bool>) -> anyhow::Result<i64> {
        let mut result: i64 = 0;
        let use_delimiter = use_delimiter.unwrap_or(true);
        let mut is_enabled: bool = true;

        let re =
            Regex::new(r"mul\(\b(?<num1>\d{1,3})\b,\b(?<num2>\d{1,3})\b\)|(?<ctrl>do[n\'t]*\(\))")
                .context("Error compliing the regex")?;
        for line in lines {
            for capture in re.captures_iter(&line) {
                let control = capture.name("ctrl").map_or("", |x| x.as_str());
                match control {
                    "do()" => {
                        if use_delimiter {
                            is_enabled = true;
...
mod tests {
    ...
    fn solve_works() {
        // Arrange
        let lines =
            vec![String::from(r"xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))")];

        // Act
        let result = Solution::solve(&lines, Some(true));

        // Assert
        assert!(result.is_ok_and(|x| x == 48));
    }
}
